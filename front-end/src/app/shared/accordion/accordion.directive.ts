...
export class AccordionDirective implements OnInit {
    ...
    closeOtherLinks(openLink: AccordionLinkDirective): void {
        this.countState++;
        if ((openLink.group !== 'sub-super-toggled' || openLink.group !== 'sub-toggled' || openLink.group !== 'main-toggled') && this.countState === 1) {
        ...
        this.navlinks.forEach((link: AccordionLinkDirective) => {
            ...
            } else if (openLink.group === 'sub-super-toggled') {
                if (link !== openLink && (link.group === 'sub-super-toggled' || openLink.group !== 'sub-super-toggled')) {
                    link.open = false;
                }
            } else {
                if (link !== openLink) {
